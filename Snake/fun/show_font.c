...
{
	int score_ = score;
	char buf[7];					   // 增加数组大小以容纳字符串终止符
	memset(buf, '0', sizeof(buf) - 1); // 初始化前5个元素为'0'，最后一个元素为'\0'
	buf[6] = '\0';					   // 确保字符串正确终止
	for (int i = 2; i >= 0 && score_ > 0; i--)
	{								  // 从后向前填充数字
		buf[i] = '0' + (score_ % 10); // 将数字转换为对应的字符
		score_ /= 10;
	}
	...
	// 创建一个画板（点阵图）
	bitmap *bm = createBitmapWithInit(70, 26, 4, getColor(0, 13, 131, 237));

	fontPrint(f1, bm, 3, 2, buf, getColor(0, 3, 35, 91), 120);
	show_font_to_lcd(lcd1->mp, 680, 180, bm);
